...
export class Worker {
 ...
 /**
  ...
  //added feature update contact
  public updateContact(inID : string, inContact : IContact):Promise<string | void>{
    console.log("Contacts.Worker.updateContact()", inContact, inID);
    return new Promise((inResolve, inReject) =>{
      this.db.update(
        {_id:inID},inContact,{},(inError, inNumReplaced: number)=>{
          if (inError) {
            console.log("Contacts.Worker.updateContact(): Error", inError);
            inReject(inError);
          } else {
            console.log("Contacts.Worker.updateContact(): Ok", inNumReplaced);
            inResolve();
          }
        }
      );
